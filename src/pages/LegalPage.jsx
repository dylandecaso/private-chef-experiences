...
export default function LegalPage({
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-hairline bg-paper/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
          <Link to="/" className="group flex min-w-0 items-center gap-2.5">
            <svg
              viewBox="0 0 24 24"
              className="h-8 w-8 shrink-0 fill-green transition-transform duration-300 group-hover:scale-105"
              aria-hidden="true"
            >
              <path
                fill="currentColor"
            ...
            <span className="truncate font-serif text-base font-medium tracking-[0.18em] text-green sm:text-lg sm:tracking-[0.2em]">
              {t('brand.name')}
            </span>
          </Link>
          <Link
            to="/"
            className="shrink-0 whitespace-nowrap text-xs tracking-wide text-warm-muted transition-colors hover:text-green sm:text-sm"
          >
            ← Back to Home
          </Link>
        </div>
      ...
      <main className="paper-texture bg-paper">
        <div className="mx-auto max-w-4xl px-5 pb-10 pt-16 lg:px-8 lg:pt-24">
          <p className="text-xs uppercase tracking-[0.4em] text-warm-muted">{eyebrow}</p>
          <h1 className="mt-4 font-serif text-4xl italic text-green sm:text-5xl">{title}</h1>
          <p className="mt-4 text-sm text-warm-muted">Last updated: {updated}</p>
